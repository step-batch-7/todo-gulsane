...
const deleteToDo = function (req, res) {
  ...
  toDoListCollection.deleteToDo(toDoId);
  writeDataFile();
  res.statusCode = 200;
  res.end();
};
...
const deleteTask = function (req, res) {
  ...
  toDoListCollection.deleteToDoTask(toDoId, taskId);
  writeDataFile();
  res.statusCode = 200;
  res.end();
};

const addNewTaskToToDo = function (req, res) {
  const {toDoId, title} = req.body;
  const task = toDoListCollection.addNewTaskToToDo(toDoId, title);
  writeDataFile();
  req.statusCode = 200;
...
const toggleTaskStatus = function (req, res) {
  const {toDoId, taskId} = req.body;
  const status = toDoListCollection.toggleTaskStatus(toDoId, taskId);
  writeDataFile();
  req.statusCode = 200;
  res.end(JSON.stringify({status}));
};

const changeTaskTitle = function (req, res) {
...
const changeToDoTitle = function (req, res) {
...
app.post('/addNewTask', addNewTaskToToDo);
app.post('/changeTaskTitle', changeTaskTitle);
app.post('/changeToDoTitle', changeToDoTitle);
app.get('/toDoList', serveToDoListCollection);
