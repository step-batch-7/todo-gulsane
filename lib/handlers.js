...
const addToDoList = function (req, res) {
  const {username, usersDataStore} = req;
...
const verifyUser = function (req, res) {
  ...
  req.app.locals.loginMessage = 'Wrong username/password';
  res.redirect('login.html');
};
...
const ensureLoggedIn = function (req, res, next) {
  ...
  if (username) {
    ...
    usersDataStore.initialize();
    req.usersDataStore = usersDataStore;
    req.username = username;
    return next();
  }
  req.app.locals.loginMessage = 'Login Again';
  res.redirect('login.html');
};
