...
const addNewTaskToToDo = function(req, res) {
  ...
  const task = toDoList.addNewTaskToToDo(toDoId, title);
  writeDataFile();
  req.statusCode = 200;
...
const toggleTaskStatus = function(req, res) {
  const { toDoId, taskId } = req.body;
  const status = toDoList.toggleTaskStatus(toDoId, taskId);
  writeDataFile();
  req.statusCode = 200;
...
const changeTaskTitle = function(req, res) {
  ...
  toDoList.changeTaskTitle(toDoId, taskId, newTitle);
  writeDataFile();
  res.statusCode = 200;
  res.end();
};
...
const changeToDoTitle = function(req, res) {
  ...
  toDoList.changeToDoTitle(toDoId, newTitle);
  writeDataFile();
  res.statusCode = 200;
  res.end();
};
